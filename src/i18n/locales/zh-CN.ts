...
export const zhCN: TranslationKeys = {
  ...
  modals: {
    ...
    launcherInstall: {
      ...
      cardTitlePrepareWithFnm: '通过 fnm 准备 Node.js LTS',
      cardTitleInstallFnm: '先安装 fnm',
      ...
      cardDescPrepareWithFnm: 'Termy 检测到了 fnm，因此这条命令会先通过 fnm 安装 Node.js LTS，再用 npm 安装该启动器。',
      cardDescInstallFnm: 'Termy 暂未在 Obsidian 环境中检测到 Node.js、npm 或 fnm。请先安装 fnm，重启 Obsidian 后再重新检测。',
      runtimeTitle: '运行环境状态',
      runtimeDescNpmReady: 'npm 已可用，Termy 可以直接安装该启动器。',
      runtimeDescFnmReady: 'npm 暂不可用，但 fnm 已可用。Termy 可以先用 fnm 安装 Node.js LTS。',
      runtimeDescFnmMissing: 'Obsidian 当前还无法使用 Node.js、npm 或 fnm。请先安装 fnm 或 Node.js LTS，重启 Obsidian 后再试。',
