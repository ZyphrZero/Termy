...
### Added
- Unified the Agent Panel on ACP (Agent Client Protocol), replacing per-CLI private protocol adapters with a single ACP-based implementation.
  - Deleted commands: `startAcpAgent`, `stopAcpAgent`, `submitPromptToActiveAcpAgent`, `cancelActiveAcpAgentTurn`, `startMockAgentStream`, `stopMockAgentStream`, `attachIdeBridgeAgentSource`.
