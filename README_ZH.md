...
[![Version](https://img.shields.io/badge/version-1.3.2-7c3aed?style=for-the-badge)](./manifest.json)
...
- 如果只是在本机的 `cmd` 或 PowerShell 中打开 Claude Code，它只是一个脱离 Obsidian 的普通本地终端进程，无法自动知道 Obsidian 当前打开的笔记、当前 vault/workspace 目录，或编辑器里的选区。
- 通过 Termy 在 Obsidian 内打开 Claude Code 终端时，会接入 Termy 的 IDE bridge；Claude Code 可以获取活动笔记路径、vault/workspace 目录，以及当前或最近一次编辑器选区。
- 从 Termy 启动的 Agent CLI 会通过环境变量收到轻量的上下文快照路径；Codex CLI 会通过启动命令接收上下文说明文件指针，OpenCode 则会自动接入 Termy 的 IDE bridge。
- Termy 会把共享上下文快照写到 Termy 插件目录：`.obsidian/plugins/<plugin-id>/agent-context/obsidian-context.json`。
- Agent 上下文快照可包含活动文件、当前选区、已打开文件，以及 vault/workspace 元数据。
- Agent 上下文接力不需要 MCP 注册，也不会修改全局 CLI 配置。
...
- **桥接**：兼容 Claude Code/OpenCode 的 IDE bridge，以及供 Codex CLI 和其他终端 Agent 使用的共享上下文快照。
